...
if [[ "$(docker images -q ${FULL_IMAGE_NAME}:${TAG_NAME} 2> /dev/null)" == "" ]]; then
  ...
  #echo "maven"
  docker run -v /var/maven:/var/maven/.m2 -v "$(pwd)":/opt/maven -w /opt/maven --rm -u 110 -e MAVEN_CONFIG=/var/maven/.m2 maven:3.6-jdk-11 mvn -Duser.home=/var/maven clean install
  cd ..
  mkdir -p target
