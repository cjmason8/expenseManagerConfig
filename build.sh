...
cd expenseManager
echo "$PWD"
docker run --rm -v "/jenkinsHome/workspace/expense-manager-pipeline/expenseManager":/usr/src/mymaven -u 0:0 -v "$HOME/.m2":/var/maven/.m2 -e MAVEN_CONFIG=/var/maven/.m2 -w /usr/src/mymaven maven:3.6.1-jdk-12 mvn -Duser.home=/var/maven clean install --no-transfer-progress
cd ..
mkdir -p target
