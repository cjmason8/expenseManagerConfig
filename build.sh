...
cd expenseManager
echo "$PWD"
docker run --rm -v "/jenkinsHome/workspace/expense-manager-pipeline/expenseManager":/usr/src/mymaven -v "$HOME/.m2":/home -w /usr/src/mymaven maven:3.6.0-jdk-12-alpine mvn clean install
cd ..
mkdir -p target
